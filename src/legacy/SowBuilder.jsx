...
/* ─────────────────────────────────────────────
   SUPABASE CONFIG
   Pulled from env vars (see .env.example) — never hardcode keys here,
   even the anon key, so this file stays safe to open-source and the
   project can point at a different Supabase project per environment.
───────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────
   SCOPE COLUMN
───────────────────────────────────────────── */
const ScopeColumn = (props) => {
  const { icon: Icon, label, accent, items, onAdd, onRemove, onEdit } = props;
  return (
    ...
    <>
      ...
      <nav className={`nav ${scrolled ? 'scrolled' : ''} print-hide`}>
        ...
        <div style={{ display:'flex', gap:10, alignItems:'center' }}>
          <button className="nav-btn print-hide" onClick={() => navigate('/legacy/dashboard')} style={{ background:'transparent', border:'1px solid var(--border)', color:'var(--gold-dim)' }}>Dashboard</button>
